function outTable = SeedFmtsOneFile(wavFileName, textGridFileName, whichTier, gender, fmtRefs, skip_list, nPoints, win, step, subjName, smooth)
% Seeding method of formant analysis for one sound file. (Chen, Whalen & Shadle, 2019). 
%  
...
if nargin < 11 || isempty(smooth),  smooth = 0; end
if nargin < 10 || isempty(subjName), subjName = 'subj'; end
if nargin < 9 || isempty(step), step = 0.002; end
if nargin < 8 || isempty(win), win = 0.050; end
if nargin < 7 || isempty(nPoints), nPoints = 10; end
if nargin < 6 || isempty(skip_list), skip_list = txt2cell([codePath sep 'skip_list.txt']); end
if nargin < 5 || isempty(fmtRefs), fmtRefs = readtable([codePath '/formant_refs.csv']); end 
if nargin < 4 || isempty(gender), gender ='M'; end
if nargin < 3 || isempty(whichTier), whichTier = 1; end

[~, fn, ~] = fileparts(wavFileName); 
TG = ReadTextGrid(textGridFileName); tier = TG(whichTier); segs = tier.segs; labs = tier.labs;
nlabs = numel(labs); 
nFmts = 3; nPoles  = 14;
F4ref = NaN; F5ref = NaN; 
p = fileparts(mfilename('fullpath'));  praatPath = [p sep 'praat']; 
fmtHeader = genFmtHeader(nFmts, nPoints,  smooth);
FmtMAT = NaN(nlabs, 3 * nPoints); 
labIndex = (1:nlabs)';
for i = 1:nlabs
    lab = labs{i}; 
    if isempty(lab), continue;end
    if any(ismember(skip_list, lab)), continue;end
    lab = upper(stripNum(lab));
    t = segs(i,:); dur = t(2)-t(1);
    if dur < win+2*step, continue; end
    if strcmpi(gender, 'M'), isFemale = 0; else, isFemale = 1;end
    ix1 = ismember(fmtRefs.Sex, gender); ix2 = ismember(fmtRefs.Vowel, lab); ind = find(ix1 & ix2); 
    if isempty(ind) 
        F1ref = []; F2ref = []; F3ref = []; 
    else
        ind = ind(1); 
        F1ref = fmtRefs(ind,:).F1;  F2ref = fmtRefs(ind,:).F2; F3ref = fmtRefs(ind,:).F3;
    end
    try
        [time, fmts, amps] = PraatFmts_ref(wavFileName, t(1), t(2), F1ref, F2ref, F3ref, ...
            win, step, isFemale, nPoles, nFmts, F4ref, F5ref, praatPath);    
        if isnan(time), continue;end
        f1f2f3 = interpLine(fmts, nPoints);
        if smooth,  f1f2f3 = applySmooth(f1f2f3,smooth); end
        FmtMAT(i,:) = f1f2f3(:)';
    catch
        continue;
    end
end
nanidx = all(isnan(FmtMAT),2); 
FNcol = repmat({fn}, sum(~nanidx),1);
SubjCol = repmat({subjName}, sum(~nanidx),1);
labs1 = labs(~nanidx,:); 
segs = segs(~nanidx, :); t1 = segs(:,1); t2 = segs(:,2);
FmtTable= array2table(FmtMAT(~nanidx, :), 'variablenames', fmtHeader); 
labIndex = labIndex(~nanidx, :); n = numel(labIndex);
prelabs1 = cell(n,1); nextlabs1 = cell(n,1);
for i = 1:n
    k = labIndex(i);
    if k ==1, prelab = []; else, prelab = labs{k-1}; end
    if k == nlabs, nextlab = []; else, nextlab = labs{k+1}; end
    prelabs1{i} = prelab; nextlabs1{i} = nextlab; 
end
metaTable = table(SubjCol, FNcol, labIndex, t1, t2, labs1, prelabs1, nextlabs1, 'variablenames', {'Subject', 'FileName', 'LabelIndex', 'StartTime', 'EndTime', 'Label', 'PreLabel1', 'NextLabel1'}); 
outTable = [metaTable, FmtTable];

return
end
function f1f2f3 = applySmooth(f1f2f3, smooth)
sf1f2f3 = f1f2f3;
if smooth ==1, s = []; else, s = smooth; end
for i = 1:3
    f = f1f2f3(:,i);
    sf = smoothn(f,s);
    sf1f2f3(:,i) = sf;
...
function header = genFmtHeader(nFmts, nPoints,  ifSmooth)
header = cell(0);
for i = 1:nFmts
    if ifSmooth, fmtLetter = ['sF' num2str(i)]; else, fmtLetter = ['F' num2str(i)]; end
    for j = 1:nPoints
        header{end+1} = [fmtLetter  '_T' num2str(j)]; 
    end
end
return
end
function label = stripNum(label)
label= regexprep(label, '\d+$', '');
return
end
function [t, fmts, amps] = PraatFmts_ref(soundFName, start_time, end_time, F1ref, F2ref, F3ref, ...
    win, step, isFemale, nPoles, nTracks, F4ref, F5ref, praatPath)
%PraatFmts_ref  -- compute f1~3 using Praat formant tracking with references
% Default settings are 'PRAAT-default'.
% OUTPUT: 
%   t : nFrames x 1  time array in sec.
%   fmts = nFrames x [F1  F2  F3]
%   amps = nFrames x [a1 a2 a3] (in dB)
% Weirong Chen  March-12-2020
sep = filesep;
if nargin < 1, eval('help PraatFmts_ref'); return; end
if nargin < 2 || isempty(start_time), start_time = 0;end 
if nargin < 3 || isempty(end_time), end_time = 0; end % 0 = end of sound file
if nargin < 4 || isempty(F1ref) || isnan(F1ref), F1ref=500; end
if nargin < 5 || isempty(F2ref) || isnan(F2ref), F2ref=1650; end
if nargin < 6 || isempty(F3ref) || isnan(F3ref), F3ref=2750; end
if nargin < 7 || isempty(win), win = 0.050;end
if nargin < 8 || isempty(step), step = 0.002;end
if nargin < 9 || isempty(isFemale), isFemale = 1;end
if nargin < 10 || isempty(nPoles), nPoles=14; end
if nargin < 11 || isempty(nTracks), nTracks=3; end
if nargin < 12 || isempty(F4ref) || isnan(F4ref), F4ref=3850; end
if nargin < 13 || isempty(F5ref) || isnan(F5ref), F5ref=4950; end
if nargin < 14 || isempty(praatPath)
    p = fileparts( mfilename('fullpath'));  praatPath = [p sep 'praat']; 
end
t=NaN(1,1); fmts=NaN(1, nTracks); amps=NaN(1,nTracks);
if end_time > 0 && end_time - start_time < win+step, return;end  % if the duration is smaller than a window+step, then skip it. 
if isFemale, max_formant_freq = 5500; else, max_formant_freq = 5000; end
howManyVarInRow = 7; %'t' 'f1' 'f2' 'f3' 'a1' 'a2' 'a3'
ifWriteResults2txt = 0;

[fp, f, e] = fileparts(soundFName);
if isempty(fp), soundFName = fullfile(pwd,soundFName); end

praatScriptFn = 'praat_LPC_burg_MATLAB.praat';  PraatScriptFName=[praatPath sep praatScriptFn];

% if ispc, praatExePath = [praatPath sep 'windows']; praatFn = 'Praat.exe';
% else, praatExePath = [praatPath sep 'unix']; praatFn = 'Praat';
% end
praatExe= getPraatExe(praatPath);
osLine = sprintf('"%s" --run "%s" "%s" %0.4f %0.4f %s %s %d %d %d %d %d 1 %d %d %d %d', ...
    praatExe, PraatScriptFName, soundFName, ...
    start_time, end_time, num2str(win), num2str(step), F1ref, F2ref, F3ref, F4ref, F5ref, max_formant_freq, nPoles, nTracks, ifWriteResults2txt); 
[s,r] = system(osLine);
if s
    fprintf('Praat error for %s\n', soundFName);
    t=NaN(1,1); fmts=NaN(1, nTracks); amps=NaN(1,nTracks);
    return
end
% Parse Praat output:
splitted = split(r,' '); q = splitted(1:end-1);
k = cellfun(@str2num,q,'UniformOutput', false); v = cell2mat(k);
%----------
tfmtamps=reshape(v,howManyVarInRow , [])'; %'t' 'f1' 'f2' 'f3' 'a1' 'a2' 'a3' 
t = tfmtamps(:,1); fmts = tfmtamps(:,2:4); amps = tfmtamps(:,5:7); 

end % end of main function

function praatExe = getPraatExe(praatPath)
sep = filesep;
if ismac
    praatExePath = [praatPath sep 'mac']; 
    praatFn = 'Praat.app/Contents/MacOS/Praat';
elseif isunix
    praatExePath = [praatPath sep 'unix']; 
    praatFn = 'praat';
elseif ispc
    praatExePath = [praatPath sep 'windows']; 
    praatFn = 'Praat.exe';
else
    disp('Platform not supported')
end

praatExe = fullfile(praatExePath,praatFn);
return
end
function OutputLine=interpLine(InputLine, density, method)
...
%  
% Weirong Chen   May-19-2014

n=size(InputLine,1);
if nargin<3, method='spline';end
if nargin<2, density=50; end
%if n>density, OutputLine=InputLine; return; end;
nDims=size(InputLine,2);
index = 1:n;
index2= linspace(1,n,density);
OutputLine=NaN*zeros(density, nDims);
if n == 1, OutputLine = repmat(InputLine, density,1);return;end
for i=1:nDims
    vector=InputLine(:,i);
    vector2 = interp1(index, vector, index2, method);
    OutputLine(:,i)=vector2;
end% for i=1:nDims
end
%%
